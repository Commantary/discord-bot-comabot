...
module.exports = function (client, options) {
  ...
  function play (msg, suffix) {
    ...
    if (msg.member.voiceChannel === undefined) return msg.channel.sendMessage(wrap('Tu n\'es pas dans un salon vocale.'))

    // Make sure the suffix exists.
    if (!suffix) return msg.channel.sendMessage(wrap('Aucune vidéo spécifié'))
    ...
    if (queue.length >= MAX_QUEUE_SIZE) {
      return msg.channel.sendMessage(wrap('Queue maximum attente'))
    }
    ...
    msg.channel.sendMessage(wrap('Recherche...')).then(response => {
      ...
      YoutubeDL.getInfo(searchstring, ['-q', '--no-warnings', '--force-ipv4'], (err, info) => {
        ...
        // Queue the video.
        response.edit(wrap('Mis en attente: ' + info.title)).then(() => {
          queue.push(info)
  ...
  function skip (msg, suffix) {
    ...
    if (voiceConnection === null) return msg.channel.sendMessage(wrap('Aucune musique jouer.'))

    // Get the queue.
    const queue = getQueue(msg.guild.id)

    if (!canSkip(msg.member, queue)) {
      return msg.channel.sendMessage(wrap('Tu ne peux pas passer sa car tu ne l\'as pas mis.')).then((response) => {
    ...
    msg.channel.sendMessage(wrap('Passer ' + toSkip + ' !'))
  }
  ...
  function queue (msg, suffix) {
    ...
    if (voiceConnection !== null) {
      const dispatcher = voiceConnection.player.dispatcher
      queueStatus = dispatcher.paused ? 'Mis en pause' : 'Jouer'
    }
  ...
  function pause (msg, suffix) {
    ...
    if (!isAdmin(msg.member)) { return msg.channel.sendMessage(wrap('Tu n\'es pas autorisé a utiliser cela.')) }

    // Pause.
    msg.channel.sendMessage(wrap('Playback mis en pause.'))
    const dispatcher = voiceConnection.player.dispatcher
  ...
  function leave (msg, suffix) {
    if (isAdmin(msg.member)) {
      const voiceConnection = client.voiceConnections.find(val => val.channel.guild.id == msg.guild.id)
      if (voiceConnection === null) return msg.channel.sendMessage(wrap('Je suis dans aucun salon.'))
      // Clear the queue.
      const queue = getQueue(msg.guild.id)
      ...
      msg.channel.sendMessage(wrap('Tu n\'as pas les permissions pour utiliser cela'))
    }
  }
  ...
  function clearqueue (msg, suffix) {
    if (isAdmin(msg.member)) {
      ...
      queue.splice(0, queue.length)
      msg.channel.sendMessage(wrap('Queue vider!'))
    } else {
      msg.channel.sendMessage(wrap('Tu n\'as pas les permissions pour utiliser cela'))
    }
  }
  ...
  function resume (msg, suffix) {
    ...
    if (!isAdmin(msg.member)) { return msg.channel.sendMessage(wrap('Tu n\'as pas les permissions pour utiliser cela')) }

    // Resume.
    msg.channel.sendMessage(wrap('Playback remis.'))
    const dispatcher = voiceConnection.player.dispatcher
  ...
  function volume (msg, suffix) {
    ...
    if (suffix > 200 || suffix < 0) {
      return msg.channel.sendMessage(wrap('Volume out of range!')).then((response) => {
        response.delete(5000)
      })
    }

    msg.channel.sendMessage(wrap('Volume mis à ' + suffix))
  ...
  function executeQueue (msg, queue) {
    ...
    if (queue.length === 0) {
      msg.channel.sendMessage(wrap('Playback finis.'))
    ...
    new Promise((resolve, reject) => {
      ...
      // Play the video.
      msg.channel.sendMessage(wrap('En cours: ' + video.title)).then(() => {
