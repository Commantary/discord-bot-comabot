...
module.exports = function (client, options) {
	...
	function play(msg, suffix) {
		...
		if (!suffix) return msg.channel.sendMessage(wrap('Aucune video spécifié.'));
		...
		if (queue.length >= MAX_QUEUE_SIZE) {
			return msg.channel.sendMessage(wrap('Maximum de la file attente !'));
		}
		...
		msg.channel.sendMessage(wrap('Searching...')).then(response => {
			...
			YoutubeDL.getInfo(searchstring, ['-q', '--no-warnings', '--force-ipv4'], (err, info) => {
				...
				if (err || info.format_id === undefined || info.format_id.startsWith('0')) {
					return response.edit(wrap('Video invalid ! :('));
				}
	...
	function skip(msg, suffix) {
		...
		if (!canSkip(msg.member, queue)) return msg.channel.sendMessage(wrap('Tu ne peux pas passer cette musique :(')).then((response) => {
	...
	function pause(msg, suffix) {
		...
		if (!isAdmin(msg.member))
			return msg.channel.sendMessage(wrap('Tu n\'est pas autorisé a utilisé cela.'));
	...
	function leave(msg, suffix) {
		if (isAdmin(msg.member)) {
			...
			msg.channel.sendMessage(wrap('Tu n\'as pas les permissions !'));
		}
	}
	...
	function clearqueue(msg, suffix) {
		if (isAdmin(msg.member)) {
			...
			queue.splice(0, queue.length);
			msg.channel.sendMessage(wrap('File vider'));
		} else {
			msg.channel.sendMessage(wrap('Tu n\'as pas les permissions !'));
		}
	}
	...
	function resume(msg, suffix) {
		...
		// Resume.
		msg.channel.sendMessage(wrap('Playback repris.'));
		const dispatcher = voiceConnection.player.dispatcher;
	...
	function volume(msg, suffix) {
		...
		if (!isAdmin(msg.member))
			return msg.channel.sendMessage(wrap('Tu n\es pas autorisé a utilisé cela.'));

		// Get the dispatcher
		const dispatcher = voiceConnection.player.dispatcher;

		if (suffix > 200 || suffix < 0) return msg.channel.sendMessage(wrap('Volume au max !')).then((response) => {
			response.delete(5000);
		});

		msg.channel.sendMessage(wrap("Volume mis a " + suffix));
